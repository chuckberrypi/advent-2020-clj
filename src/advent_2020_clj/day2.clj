(ns advent-2020-clj.day2)

(def day-two-input "1-13 r: gqdrspndrpsrjfjx
...
(def sample-line (first (lines day-two-input)))

(defn line-to-map [line]
  (let [[rule-string pw] (clojure.string/split line #":")
        [min-max-str chr] (clojure.string/split rule-string #" ")
        [minc maxc] (clojure.string/split min-max-str #"-")
        min (Integer/parseInt minc)
        max (Integer/parseInt maxc)]

    {:pw (clojure.string/trim pw)
     :chr chr
     :min min
     :max max}))

(defn valid? [{:keys [pw chr min max]}]
  (let [count (count (re-seq (re-pattern chr) pw))]
    (and (<= count max)
         (>= count min))))
...
(def part2-sample-lines ["1-3 a: abcde"
                         "1-3 b: cdefg"
                         "2-9 c: ccccccccc"])

(defn new-valid? [{:keys [pw chr min max]}]
  (let [c (first (seq chr))
        pws (seq pw)
        fst (nth pws (dec min))
        snd (nth pws (dec max))]
